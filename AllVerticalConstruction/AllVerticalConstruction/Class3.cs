...
namespace AllVerticalConstruction
{
    [BepInPlugin("bifrom.com.DSP.AllVerticalConstruction", "[戴森球计划] 制造厂的垂直建造 by 丰有珏", "1.0.6")]
    // 限制只有戴森球计划可以使用当前插件
    [BepInProcess("DSPGAME.exe")]


    public class AllVerticalConstructionConfig
    {
        public int ID = 0;
        public Vector3 LapJoint;
        public float ColliderDataOffset;
        public AllVerticalConstructionConfig(int id, Vector3 lapJoint, float colliderDataOffset)
        {
            this.ID = id;
    ...
    public class AllVerticalConstruction : BaseUnityPlugin
    {
        public static ConfigEntry<bool> IsResetNextIds;
        public static ConfigEntry<string> AllVerticalConstructionJson;
        public static List<AllVerticalConstructionConfig> AllVerticalConstructionConfigs = new List<AllVerticalConstructionConfig>();
        public static AssemblerComponentEx assemblerComponentEx = new AssemblerComponentEx();
        public static BepInEx.Logging.ManualLogSource mylog = BepInEx.Logging.Logger.CreateLogSource("AllVerticalConstruction");
        ~AllVerticalConstruction()
        {
        ...
        public string AllVerticalConstructionConfigsToString()
        {
            string ret = "\n";
            for (int i = 0; i < AllVerticalConstructionConfigs.Count; i++)
            {
                if (ret != "\n")
                {
                    ret += ",";
                }
                ret += AllVerticalConstructionConfigs[i].ID + "|";
                ret += AllVerticalConstructionConfigs[i].LapJoint.ToString() + "|";
                ret += AllVerticalConstructionConfigs[i].ColliderDataOffset;
            }
            return ret;
        }
        ...
        void Start()
        {
            AllVerticalConstructionConfigs.Add(new AllVerticalConstructionConfig(2303, new Vector3(0, 5.1f, 0), -3));
            AllVerticalConstructionConfigs.Add(new AllVerticalConstructionConfig(2304, new Vector3(0, 5.1f, 0), -3));
            AllVerticalConstructionConfigs.Add(new AllVerticalConstructionConfig(2305, new Vector3(0, 5.1f, 0), -3));
            AllVerticalConstructionConfigs.Add(new AllVerticalConstructionConfig(2302, new Vector3(0, 4.3f, 0), -3));
            AllVerticalConstructionConfigs.Add(new AllVerticalConstructionConfig(2309, new Vector3(0, 7.0f, 0), -3));
            AllVerticalConstructionConfigs.Add(new AllVerticalConstructionConfig(2308, new Vector3(0, 16.0f, 0), -3));

            IsResetNextIds = Config.Bind("config", "IsResetNextIds", false, "在加载存档的时候重新计算建筑物的叠加关系 true需要重新计算 重新计算的时候会有一定的卡顿");
            AllVerticalConstructionJson = Config.Bind("config", "AllVerticalConstructionJson", AllVerticalConstructionConfigsToString(), "建筑间隔信息 ");

            Harmony.CreateAndPatchAll(typeof(AllVerticalConstruction));
        }
